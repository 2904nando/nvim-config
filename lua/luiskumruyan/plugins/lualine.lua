...
local fmt_stat = {
	...
	on_click = function()
		...
		vim.ui.select({ 2, 4, 8 }, { prompt = "Local number of spaces per tab" }, function(spaces)
			if spaces ~= nil then
				vim.opt_local.tabstop = spaces
				vim.opt_local.softtabstop = spaces -- For editing
				vim.opt_local.shiftwidth = spaces -- For autoindent

				vim.notify("Local number of spaces per tab: " .. vim.opt_local.tabstop:get(), vim.log.levels.INFO, {
					title = "Set local spaces",
				})
			end
		end)
	end,
